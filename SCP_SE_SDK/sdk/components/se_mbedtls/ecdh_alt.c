                                           ...
                                           mbedtls_ecp_restart_ctx *rs_ctx)
{
    ...
    kss_sscp_derive_key_t dCtx;
    size_t coordinateLen     = (grp->pbits + 7u) / 8u;
    size_t coordinateBitsLen = grp->pbits;

    kss_sscp_object_t pEcdhPubKey;
    kss_sscp_object_t sharedSecret;
    kss_sscp_object_t *pSecretKey = NULL;
    bool bFreeSecretKey = false;

    size_t keySize           = KSS_ECP_KEY_SZ(coordinateLen);
    uint8_t *pubKey          = mbedtls_calloc(keySize, sizeof(uint8_t));

    if (CRYPTO_InitHardware() != kStatus_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_init(&pEcdhPubKey, &g_keyStore) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_allocate_handle(&pEcdhPubKey,
                                                   0u,
                                                   kKSS_KeyPart_Public,
                                                   kKSS_CipherType_EC_NIST_P,
                                                   3u * coordinateLen,
                                                   KSS_KEYPROP_OPERATION_KDF) !=
               kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    ...
    } else if (KSS_KEY_STORE_SET_KEY(&pEcdhPubKey,
                                     (const uint8_t *) pubKey,
                                     keySize,
                                     coordinateBitsLen,
                                     kKSS_KeyPart_Public) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_init(&sharedSecret, &g_keyStore) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_allocate_handle(&sharedSecret,
                                                   0u,
                                                   kKSS_KeyPart_Default,
                                                   kKSS_CipherType_AES,
                                                   coordinateLen,
                                                   KSS_KEYPROP_OPERATION_NONE
                                                   ) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    }
    ...
    if ((d->s == MBEDTLS_MPI_S_HAVE_OBJECT) && (d->n == MBEDTLS_MPI_N_HAVE_OBJECT)) {
        pSecretKey = (kss_sscp_object_t *) (uintptr_t) d->p;
        bFreeSecretKey = true;
    } else {
        kss_sscp_object_t secretKey;
        pSecretKey = &secretKey;

        /* Load secret key into KSS */
        if (kss_sscp_key_object_init(pSecretKey, &g_keyStore) != kStatus_KSS_Success) {
            ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
        } else if (kss_sscp_key_object_allocate_handle(pSecretKey,
                                                       0u,
            ...
            ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
        } else if (KSS_KEY_STORE_SET_KEY(pSecretKey, (const uint8_t *) pubKey, coordinateLen,
                                         coordinateBitsLen,
                                         kKSS_KeyPart_Private) != kStatus_KSS_Success) {
            return MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
        }
    }

    if (kss_sscp_derive_key_context_init(&dCtx, &g_kssSession, pSecretKey, kAlgorithm_KSS_ECDH,
                                         kMode_KSS_ComputeSharedSecret) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_asymmetric_dh_derive_key(&dCtx, &pEcdhPubKey,
                                                 &sharedSecret) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_store_get_key(&g_keyStore, &sharedSecret, pubKey, &coordinateLen,
                                          &coordinateBitsLen,
                                          (kss_key_part_t) NULL) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if ((ret = mbedtls_mpi_read_binary(z, pubKey, coordinateLen)) != 0) {
    } else {
        ret = 0;
    }
    (void) kss_sscp_derive_key_context_free(&dCtx);
    (void) KSS_KEY_OBJ_FREE(&pEcdhPubKey);
    (void) KSS_KEY_OBJ_FREE(&sharedSecret);
    mbedtls_platform_zeroize(pubKey, keySize);
    mbedtls_free(pubKey);

    if (bFreeSecretKey) {
        (void) KSS_KEY_OBJ_FREE(pSecretKey);
    }
                             ...
                             void *p_rng)
{
    ...
    do {
        ...
        if (ctx->isKeyInitialized == false) {
            if (kss_sscp_key_object_init(&ctx->key, &g_keyStore) != kStatus_KSS_Success) {
                (void) KSS_KEY_OBJ_FREE(&ctx->key);
                break; \
            }
            /* Allocate key handle */
#if (defined(KW45_A0_SUPPORT) && KW45_A0_SUPPORT)
            if (kss_sscp_key_object_allocate_handle(&ctx->key,
                                                    0u,
                                                    kKSS_KeyPart_Pair,
                                                    kKSS_CipherType_EC_NIST_P,
                                                    3u * coordinateLen,
                                                    KSS_PUBLIC_KEY_PART_EXPORTABLE) !=
                kStatus_KSS_Success)
#else
            if (kss_sscp_key_object_allocate_handle(&ctx->key,
                                                    0u,
        ...
        if (KSS_ECP_GENERATE_KEY(&ctx->key, coordinateBitsLen) != kStatus_KSS_Success) {
            break;
        }
        if (KSS_KEY_STORE_GET_PUBKEY(&ctx->key,
                                     pubKey,
                                     &keySize,
                                     &coordinateBitsLen) != kStatus_KSS_Success) {
            break;
        }
                             ...
                             void *p_rng)
{
    ...
    if (CRYPTO_InitHardware() != kStatus_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_init(&ctx->peerPublicKey, &g_keyStore) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    }
#if (defined(KW45_A0_SUPPORT) && KW45_A0_SUPPORT)
    else if (kss_sscp_key_object_allocate_handle(&ctx->peerPublicKey,
                                                 1u,
                                                 kKSS_KeyPart_Pair,
                                                 kKSS_CipherType_EC_NIST_P,
                                                 3u * coordinateLen,
             ...
             kStatus_KSS_Success)
                                                 ...
                                                 kKSS_CipherType_EC_NIST_P,
                                                 3u * coordinateLen,
                                                 KSS_KEYPROP_OPERATION_KDF) != kStatus_KSS_Success)
#endif
    {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if ((ret =
                    ...
                    mbedtls_mpi_write_binary(&ctx->ctx.mbed_ecdh.Qp.Y, &pubKey[coordinateLen],
                                             coordinateLen)) != 0) {
    } else if (KSS_KEY_STORE_SET_KEY(&ctx->peerPublicKey,
                                     (const uint8_t *) pubKey,
                                     keySize,
                                     coordinateBitsLen,
                                     kKSS_KeyPart_Public) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_init(&ctx->sharedSecret, &g_keyStore) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_object_allocate_handle(&ctx->sharedSecret,
                                                   2u,
                                                   kKSS_KeyPart_Default,
                                                   kKSS_CipherType_AES,
                                                   coordinateLen,
#if (defined(KW45_A0_SUPPORT) && KW45_A0_SUPPORT)
                                                   KSS_FULL_KEY_EXPORTABLE
#else
                                                   KSS_KEYPROP_OPERATION_NONE
#endif
                                                   ) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_derive_key_context_init(&dCtx, &g_kssSession, &ctx->key,
                                                kAlgorithm_KSS_ECDH,
                                                kMode_KSS_ComputeSharedSecret) !=
               kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_asymmetric_dh_derive_key(&dCtx, &ctx->peerPublicKey,
                                                 &ctx->sharedSecret) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    } else if (kss_sscp_key_store_get_key(&g_keyStore, &ctx->sharedSecret, pubKey, &coordinateLen,
                                          &coordinateBitsLen,
                                          (kss_key_part_t) NULL) != kStatus_KSS_Success) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    ...
    (void) kss_sscp_derive_key_context_free(&dCtx);
    (void) KSS_KEY_OBJ_FREE(&ctx->peerPublicKey);
    (void) KSS_KEY_OBJ_FREE(&ctx->sharedSecret);
...
int mbedtls_ecdh_self_test(int verbose)
{
    ...
    for (uint32_t i = 0; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++, i++) {
        if (verbose != 0) {
            (void) mbedtls_printf("  ECDH %s - #%lu: ", curve_info->name, i + 1u);
        }
