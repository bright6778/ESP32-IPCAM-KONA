    ...
    size_t sig_len)
{
    ...
    LOGI(TAG, "%s: Verify using key %08" PRIX32 "", __FUNCTION__, pax_ctx->pKSSObject->keyId);
    ...
    void *p_rng)
{
    ...
    LOGI(TAG, "%s: Signing using key '0x%08lX'", __FUNCTION__, pax_ctx->pKSSObject->keyId);
