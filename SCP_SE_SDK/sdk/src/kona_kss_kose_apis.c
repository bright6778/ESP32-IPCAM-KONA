...
/*
    ...
    in KoseSession_t struct (hostlib\hostLib\inc\kose_tlv.h)
*/
//#define KSS_USE_SCP03_THREAD_SAFETY
...
smStatus_t kss_kose_create_curve_if_needed(KoseSession_t *pSession, uint32_t curve_id);
...
#if KSSFTR_KOSE_AuthSession
static smStatus_t kose_CreateVerifyUserIDSession(
    pKoseSession_t koseSession, const uint32_t auth_id, KOSE_AuthCtx_ID_t *pin, pKosePolicy_t policy);
#endif

#if KSS_HAVE_SCP_SCP03_KSS
#if KSSFTR_KOSE_AuthECKey
static smStatus_t kose_CreateECKeySession(
    pKoseSession_t koseSession, const uint32_t auth_id, KOSE_AuthCtx_ECKey_t *pFScpCtx);
#endif

#if KSSFTR_KOSE_AuthSession
static smStatus_t kose_CreateVerifyAESKeySession(
    pKoseSession_t koseSession, const uint32_t auth_id, SCP03_AuthCtx_t *pAppletSCPCtx);
#endif
#endif

static smStatus_t kss_kose_channel_txnRaw(void *conn_ctx,
    ...
    uint8_t hasle);
#if 0
static KOSE_RSASignatureAlgo_t kose_get_rsa_sign_mode(
    kss_algorithm_t algorithm);
#endif

#if KSSFTR_KOSE_RSA && KSS_HAVE_RSA
static KOSE_RSAEncryptionAlgo_t kose_get_rsa_encrypt_mode(kss_algorithm_t algorithm);
...
static KOSE_MACAlgo_t kose_get_mac_algo(kss_algorithm_t algorithm);
#if KSSFTR_KOSE_KEY_SET || KSSFTR_KOSE_KEY_GET
static uint8_t CheckIfKeyIdExists(uint32_t keyId, pKoseSession_t session_ctx, smStatus_t *apduStatus);
#endif
static smStatus_t kss_kose_channel_txn(void *conn_ctx,
...
#if KSSFTR_KOSE_AuthECKey || KSSFTR_KOSE_AuthSession
static kss_status_t kss_session_auth_open(kss_kose_session_t *session,
...
#if KSSFTR_KOSE_RSA && KSSFTR_KOSE_KEY_SET && KSS_HAVE_RSA
static kss_status_t kss_kose_key_store_set_rsa_key(kss_kose_key_store_t *keyStore,
...
static kss_status_t kose_check_input_len(size_t inLen, kss_algorithm_t algorithm);
#endif

#if KSS_HAVE_KOSE_VER_GTE_07_02
static kss_status_t kss_kose_aead_CCMfinish(kss_kose_aead_t *context,
...
#if KSS_HAVE_KOSE_VER_GTE_07_02
typedef smStatus_t (*fp_Ec_KeyWrite_t)(pKoseSession_t session_ctx,
    pKosePolicy_t policy,
    KOSE_MaxAttemps_t maxAttempt,
    uint32_t objectID,
    KOSE_ECCurve_t curveID,
...
#endif //KSSFTR_KOSE_ECC && KSSFTR_KOSE_KEY_SET
...
#if KSS_HAVE_KOSE_VER_GTE_07_02
typedef smStatus_t (*fp_Symm_KeyWrite_t)(pKoseSession_t session_ctx,
    pKosePolicy_t policy,
    KOSE_MaxAttemps_t maxAttempt,
    uint32_t objectID,
    KOSE_KeyID_t kekID,
...
#if KSSFTR_KOSE_RSA && KSSFTR_KOSE_KEY_SET && KSS_HAVE_RSA
static smStatus_t kss_kose_LL_set_RSA_key(pKoseSession_t session_ctx,
    pKosePolicy_t policy,
    uint32_t objectID,
    uint16_t size,
    const uint8_t *p,
    ...
    const KOSE_INS_t ins_type,
    const KOSE_KeyPart_t key_part,
    const KOSE_RSAKeyFormat_t rsa_format,
    KOSE_Result_t obj_exists);

#if KSS_HAVE_KOSE_VER_GTE_07_02
typedef smStatus_t (*fp_RSA_KeyWrite_t)(pKoseSession_t session_ctx,
    pKosePolicy_t policy,
    uint32_t objectID,
    uint16_t size,
    const uint8_t *p,
    ...
    uint32_t version);
#endif //KSS_HAVE_KOSE_VER_GTE_07_02
#endif //KSSFTR_KOSE_RSA && KSSFTR_KOSE_KEY_SET && KSS_HAVE_RSA

#define KSS_KOSE_RESID_ATTESTATION_KEY 0xF0000012
#if KSS_HAVE_KOSE_VER_GTE_07_02
extern int add_taglength_to_data(
    uint8_t **buf, size_t *bufLen, KOSE_TAG_t tag, const uint8_t *cmd, size_t cmdLen, bool extendedLength);

#endif // KSS_HAVE_KOSE_VER_GTE_07_02

kss_status_t nxECKey_ReadEckaPublicKey(pKoseSession_t koseSession,
    kss_key_store_t *pHostKeyStore,
    uint8_t *pSePubEcka,
    ...
    void *connectionData)
{
    ...
    if (pAuthCtx->connType != kType_SE_Conn_Type_Channel) {
        ...
        if (1 == pAuthCtx->skip_select_applet) {
            if (pAuthCtx->auth.authType == kKSS_AuthType_None) {
                CommState.select = SELECT_NONE;
            }
            else if (pAuthCtx->auth.authType == kKSS_AuthType_SCP03) {
    ...
    defined(SMCOM_RC663_VCOM)
        lReturn = SM_RjctConnect(&(koseSession->conn_ctx), pAuthCtx->portName, &CommState, atr, &atrLen);
        ...
        lReturn = SM_Connect(&(koseSession->conn_ctx), &CommState, atr, &atrLen);
        if (lReturn != SW_OK) {
        ...
        if (1 == pAuthCtx->skip_select_applet) {
            ...
            if (HEX_EXPECTED_APPLET_VERSION == (0xFFFFFF00 & CommState.appletVersion)) {
                ...
                LOG_E("Aborting!!!");
                LOG_E("Use a library with adjusted PTMW_KOSE_Ver compile time setting");
    ...
    if (pAuthCtx->auth.authType == kKSS_AuthType_ECKey) {
        ...
        if (CommState.appletVersion == 0) {
            ...
            kss_kose_session_t *kose_session = (kss_kose_session_t *)pAuthCtx->tunnelCtx->session;
            status = Kose_API_GetVersion(&kose_session->s_ctx, appletVersion, &appletVersionLen);
    ...
    koseSession->fp_RawTXn = &kss_kose_channel_txn;
...
#else
    if (pAuthCtx->auth.authType != kKSS_AuthType_None && pAuthCtx->auth.authType != kKSS_AuthType_ID) {
        LOG_E(
            "Set the SCP to SCP03_KSS in the build configuration and "
            "recompile.!");
    }
...
#if KSSFTR_KOSE_AuthECKey || KSSFTR_KOSE_AuthSession
    if (pAuthCtx->connType == kType_SE_Conn_Type_Channel) {
        koseSession->pChannelCtx = (struct _kss_kose_tunnel_context *)pAuthCtx->tunnelCtx;
        if (koseSession->pChannelCtx->kose_session->subsystem == kType_KSS_SE_KOSE) {
    ...
    if ((application_id != 0) &&
        ...
        do {
            ...
            SM_LOCK_CHANNEL();
            retval = kss_session_auth_open(session, subsystem, application_id, connection_type, connectionData);
            SM_UNLOCK_CHANNEL();
            if (retval == kStatus_KSS_Success) {
                break;
            }
...
static kss_status_t kss_session_auth_open(kss_kose_session_t *session,
    kss_type_t subsystem,
    uint32_t auth_id,
    kss_connection_type_t connect_type,
    void *connectionData)
{
...
#endif
    pKoseSession_t koseSession = NULL;
    ...
    koseSession->conn_ctx       = conn_ctx;
    koseSession->applet_version = applet_version;

    ENSURE_OR_GO_EXIT(connectionData != NULL);
    pAuthCtx = (KOSE_Connect_Ctx_t *)connectionData;

    if ((pAuthCtx->auth.authType == kKSS_AuthType_ID) && (connect_type != kKSS_ConnectionType_Password)) {
        LOG_D("ERROR: Need both AUTHType=ID and ConnType=Password");
        goto exit;
    }
    if (((pAuthCtx->auth.authType == kKSS_AuthType_AESKey) || (pAuthCtx->auth.authType == kKSS_AuthType_ECKey)) &&
        (connect_type != kKSS_ConnectionType_Encrypted)) {
    ...
    if (pAuthCtx->session_policy) {
        if (kStatus_KSS_Success != kss_kose_create_session_policy_buffer(
    ...
    kose_policy.value     = (uint8_t *)ppolicySet;
    kose_policy.value_len = valid_policy_buff_len;
#endif

    /* Auth type is Platform UserID */

    if (pAuthCtx->auth.authType == kKSS_AuthType_ID) {
#if KSSFTR_KOSE_AuthSession
        ...
        status = kose_CreateVerifyUserIDSession(koseSession, auth_id, &pAuthCtx->auth.ctx.idobj, &kose_policy);
#else
...
#if KSS_HAVE_SCP_SCP03_KSS
    /* Auth type is ECKey */
    else if ((pAuthCtx->auth.authType == kKSS_AuthType_ECKey) && (auth_id != 0)) {
#if KSSFTR_KOSE_AuthECKey && KSSFTR_KOSE_AuthSession
        koseSession->fp_Transform = &kose_Transform;
        koseSession->fp_DeCrypt   = &kose_DeCrypt;
        status                     = kose_CreateECKeySession(koseSession, auth_id, &pAuthCtx->auth.ctx.eckey);
        if (status == SM_OK) {
            koseSession->fp_Transform = &kose_Transform_scp;
            if (kose_policy.value_len > 0) {
                status = Kose_API_ExchangeSessionData(koseSession, &kose_policy);
            }
        }
#else
    ...
    else if ((pAuthCtx->auth.authType == kKSS_AuthType_AESKey) && (auth_id != 0)) {
#if KSSFTR_KOSE_AuthSession
        koseSession->fp_Transform = &kose_Transform;
        koseSession->fp_DeCrypt   = &kose_DeCrypt;
        status                     = kose_CreateVerifyAESKeySession(koseSession, auth_id, &pAuthCtx->auth.ctx.scp03);
        if (status == SM_OK) {
            koseSession->fp_Transform = &kose_Transform_scp;
            if (kose_policy.value_len > 0) {
                status = SM_NOT_OK;
...
{
    kss_status_t retval                  = kStatus_KSS_Fail;
    pKoseSession_t koseSession         = &session->s_ctx;
    kss_policy_session_u *session_policy = (kss_policy_session_u *)connectionData;
    smStatus_t status                    = SM_NOT_OK;
    size_t valid_policy_buff_len         = 0;
    KosePolicy_t kose_policy           = {0};
    uint8_t *ppolicySet;
    uint8_t session_policies_buff[MAX_POLICY_BUFFER_SIZE];

    if (session_policy) {
        if (kStatus_KSS_Success !=
            kss_kose_create_session_policy_buffer(session_policy, &session_policies_buff[0], &valid_policy_buff_len)) {
            goto exit;
        }
        ppolicySet             = session_policies_buff;
        kose_policy.value     = (uint8_t *)ppolicySet;
        ...
        ppolicySet             = NULL;
        kose_policy.value     = NULL;
        kose_policy.value_len = 0;
    }

    status = Kose_API_RefreshSession(koseSession, &kose_policy);
    if (status == SM_OK) {
        retval = kStatus_KSS_Success;
    }
exit:
...
{
    kss_status_t retval                   = kStatus_KSS_Success;
    kss_session_prop_u32_t prop           = (kss_session_prop_u32_t)property;
    kss_s05x_sesion_prop_u32_t se050xprop = (kss_s05x_sesion_prop_u32_t)property;
    AX_UNUSED_ARG(session);
    ...
    switch (prop) {
    case kKSS_SessionProp_VerMaj:
        *pValue = PLUGANDTRUST_HOSTLIB_VER_MAJOR;
        break;
    case kKSS_SessionProp_VerMin:
        *pValue = PLUGANDTRUST_HOSTLIB_VER_MINOR;
        break;
    case kKSS_SessionProp_VerDev:
        *pValue = PLUGANDTRUST_HOSTLIB_VER_DEV;
        break;
    case kKSS_SessionProp_UIDLen:
        *pValue = 18;
        break;
    default:
        *pValue = 0;
    ...
    switch (se050xprop) {
    case kKSS_KOSE_SessionProp_CertUIDLen: {
        *pValue = 10;
        retval  = kStatus_KSS_Success;
    } break;
    default: {
...
{
    kss_status_t retval                   = kStatus_KSS_Fail;
    kss_session_prop_au8_t prop           = (kss_session_prop_au8_t)property;
    kss_s05x_sesion_prop_au8_t se050xprop = (kss_s05x_sesion_prop_au8_t)property;
    ...
    switch (prop) {
    case kKSS_SessionProp_UID:
        if (*pValueLen >= 18) {
            sm_status = Kose_API_ReadObject(&session->s_ctx, kKOSE_AppletResID_UNIQUE_ID, 0, 18, pValue, pValueLen);
        }
        else {
    ...
    switch (se050xprop) {
    case kKSS_KOSE_SessionProp_CertUID:
        if (*pValueLen >= 10) {
            uint8_t uid18[SE050_MODULE_UNIQUE_ID_LEN];
            size_t uid18Len = sizeof(uid18);

            sm_status = Kose_API_ReadObject(
                &session->s_ctx, kKOSE_AppletResID_UNIQUE_ID, 0, (uint16_t)uid18Len, uid18, &uid18Len);
            if (sm_status == SM_OK) {
                int idx = 0;
...
#ifdef KSS_USE_SCP03_THREAD_SAFETY
#if KSS_HAVE_SCP_SCP03_KSS
#if defined(USE_RTOS) && (USE_RTOS == 1)
...
#endif //#if KSS_HAVE_SCP_SCP03_KSS
#endif //#ifdef KSS_USE_SCP03_THREAD_SAFETY

    sm_status = Kose_API_CloseSession(&session->s_ctx);
...
//static kss_status_t kss_kose_key_object_get_handle_binary(
//    kss_kose_object_t *keyObject) {
//    kss_status_t retval = kStatus_KSS_Success;
    ...
    if (apiRetval == SM_OK) {
...
#if KSS_HAVE_KOSE_VER_GTE_07_02
        if (retObjectType >= kKOSE_SecObjTyp_EC_KEY_PAIR_NIST_P192 &&
            retObjectType <= kKOSE_SecObjTyp_EC_PUB_KEY_MONT_DH_448)
#else
...
#endif
        {
            ...
            if (apiRetval == SM_OK) {
                ...
                else if (retCurveId == kKOSE_ECCurve_RESERVED_ID_ECC_MONT_DH_448) {
                    keyObject->cipherType = kKSS_CipherType_EC_MONTGOMERY;
                }
#endif
                else {
                ...
                LOG_E("error in Kose_API_GetECCurveId");
        ...
        LOG_W("Error in Kose_API_ReadType. Further use of object may fail");
        if (apiRetval == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
        }
        else {
    ...
    size_t *hkdfOutputLen)
{
    kss_status_t retval                     = kStatus_KSS_Fail;
    smStatus_t status                       = SM_NOT_OK;
    uint8_t hkdfKey[KOSE_MAX_BUF_SIZE_CMD] = {
        0,
    };
    size_t hkdfKeyLen                   = sizeof(hkdfKey);
    kss_object_t *kss_derived_keyObject = (kss_object_t *)derivedKeyObject;
    KOSE_DigestMode_t digestMode;
    ...
    digestMode = kose_get_sha_algo(context->algorithm);
    ENSURE_OR_GO_EXIT(digestMode != kKOSE_DigestMode_NA);

    status = Kose_API_HKDF(&context->session->s_ctx,
    ...
    retval = kss_key_store_set_key((kss_key_store_t *)derivedKeyObject->keyStore,
        kss_derived_keyObject,
        hkdfKey,
        hkdfKeyLen,
        hkdfKeyLen * 8,
        NULL,
        0);
    ENSURE_OR_GO_EXIT(retval == kStatus_KSS_Success);
    ...
    uint16_t deriveDataLen)
{
    ...
    size_t hkdfKeyLen                   = sizeof(hkdfKey);
    kss_object_t *kss_derived_keyObject = (kss_object_t *)derivedKeyObject;
    KOSE_DigestMode_t digestMode       = kose_get_sha_algo(context->algorithm);
    uint32_t derivedKeyID               = (derivedKeyObject == NULL ? 0 : derivedKeyObject->keyId);
    uint8_t *pHkdfKey                   = hkdfKey;
    KOSE_HkdfMode_t hkdfMode =
        (context->mode == kMode_KSS_HKDF_ExpandOnly ? kKOSE_HkdfMode_ExpandOnly : kKOSE_HkdfMode_ExtractExpand);
    ...
    status = Kose_API_HKDF_Extended(&context->session->s_ctx,
        context->keyObject->keyId,
        digestMode,
        hkdfMode,
        saltData,
    ...
    kss_kose_derive_key_t *context, kss_kose_object_t *otherPartyKeyObject, kss_kose_object_t *derivedKeyObject)
{
    ...
    kss_object_t *kss_other_keyObject   = NULL;
    kss_object_t *kss_derived_keyObject = NULL;
    ENSURE_OR_GO_EXIT(context);
    ENSURE_OR_GO_EXIT(otherPartyKeyObject);
    ENSURE_OR_GO_EXIT(derivedKeyObject);
    kss_other_keyObject   = (kss_object_t *)otherPartyKeyObject;
    kss_derived_keyObject = (kss_object_t *)derivedKeyObject;
    retval                = kss_key_store_get_key(
        (kss_key_store_t *)kss_other_keyObject->keyStore, kss_other_keyObject, pubkey, &pubkeylen, &pbKeyBitLen);
    ENSURE_OR_GO_EXIT(retval == kStatus_KSS_Success);

    retval =
        kss_util_pkcs8_asn1_get_ec_public_key_index((const uint8_t *)pubkey, pubkeylen, &publicKeyIndex, &publicKeyLen);
    if (retval != kStatus_KSS_Success) {
        LOG_W("error in kss_util_pkcs8_asn1_get_ec_public_key_index");
        goto exit;
    }
    ...
    {
        if (otherPartyKeyObject->cipherType == kKSS_CipherType_EC_MONTGOMERY) {
    ...
    if (context->keyObject->keyStore->session->subsystem == derivedKeyObject->keyStore->session->subsystem) {
        status = Kose_API_ECDHGenerateSharedSecret_InObject(&context->session->s_ctx,
            context->keyObject->keyId,
            pPublicKey,
            publicKeyLen,
            ...
            invertEndiannes);
        if (status != SM_OK) {
            LOG_W("error in Kose_API_ECDHGenerateSharedSecret_InObject");
            if (status == SM_ERR_APDU_THROUGHPUT) {
                retval = kStatus_KSS_ApduThroughputError;
            }
            else {
...
#endif
        status = Kose_API_ECGenSharedSecret(&context->session->s_ctx,
            context->keyObject->keyId,
            pPublicKey,
            publicKeyLen,
            sharedsecret,
    ...
    size_t policy_buff_len)
{
    ...
    KOSE_RSAKeyFormat_t rsa_format;
    uint8_t IdExists          = 0;
    size_t keyBitLength       = 0;
    KOSE_Result_t obj_exists = kKOSE_Result_NA;
    ...
    if (keyObject->cipherType == kKSS_CipherType_RSA) {
        rsa_format = kKOSE_RSAKeyFormat_RAW;
    }
    else if (keyObject->cipherType == kKSS_CipherType_RSA_CRT) {
        rsa_format = kKOSE_RSAKeyFormat_CRT;
    }
    else {
        retval = kStatus_KSS_Fail;
        goto exit;
...
#if 0
    if (key_type == kKSS_KeyPart_Public) {
        key_part = KOSE_KeyPart_Public;
    }
    else if (key_type == kKSS_KeyPart_Private) {
        key_part = kKOSE_KeyPart_Private;
    }
    else if (key_type == kKSS_KeyPart_Pair) {
    ...
    status = Kose_API_WriteRSAKey(&keyStore->session->s_ctx,
        &kose_policy,
        keyObject->keyId,
        (U16)keyBitLen,
        KOSE_RSA_NO_p,
    ...
    if (key_type == kKSS_KeyPart_Public) {
        smStatus_t apduRetValue = SM_NOT_OK;
        retval                  = kss_util_asn1_rsa_parse_public(key, keyLen, &rsaN, &rsaNlen, &rsaE, &rsaElen);
        ENSURE_OR_GO_EXIT(retval == kStatus_KSS_Success);
        ...
        obj_exists   = (IdExists == 1) ? kKOSE_Result_SUCCESS : kKOSE_Result_FAILURE;
        ...
        status = kss_kose_LL_set_RSA_key(&keyStore->session->s_ctx,
            &kose_policy,
            keyObject->keyId,
            (U16)keyBitLength,
            KOSE_RSA_NO_p,
        ...
        smStatus_t apduRetValue = SM_NOT_OK;
        if (keyObject->cipherType == kKSS_CipherType_RSA) {
            retval = kss_util_asn1_rsa_parse_private(key,
                keyLen,
                (kss_cipher_type_t)keyObject->cipherType,
                &rsaN,
                &rsaNlen,
                NULL,
                NULL,
        ...
        if (keyObject->cipherType == kKSS_CipherType_RSA) {
            ...
            ENSURE_OR_EXIT_WITH_STATUS_ON_ERROR(
                !((rsaD == NULL) || (rsaE == NULL) || (rsaN == NULL)), retval, kStatus_KSS_Fail);
...
#if KSSFTR_KOSE_ECC && KSSFTR_KOSE_KEY_SET
static kss_status_t getEccPrivPubKeyLen(uint32_t curve_id, size_t *pubKeyLen, size_t *privKeyLen)
{
    kss_status_t retval = kStatus_KSS_Success;
    ...
    switch (curve_id) {
#if KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP
#if KSS_HAVE_EC_NIST_K
    case kKOSE_ECCurve_Secp160k1:
#endif
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool160:
#endif
    {
        *privKeyLen = 20;
        *pubKeyLen  = 41;
    } break;
#endif // KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP

#if KSS_HAVE_EC_NIST_192 || KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP
#if KSS_HAVE_EC_NIST_192
    case kKOSE_ECCurve_NIST_P192:
#endif
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool192:
#endif
#if KSS_HAVE_EC_NIST_K
    case kKOSE_ECCurve_Secp192k1:
#endif
    {
        *privKeyLen = 24;
        *pubKeyLen  = 49;
    } break;
#endif // KSS_HAVE_EC_NIST_192 || KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP

#if KSS_HAVE_EC_NIST_224 | KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP
#if KSS_HAVE_EC_NIST_224
    case kKOSE_ECCurve_NIST_P224:
#endif
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool224:
#endif
#if KSS_HAVE_EC_NIST_K
    case kKOSE_ECCurve_Secp224k1:
#endif
    {
        *privKeyLen = 28;
        *pubKeyLen  = 57;
    } break;
#endif // KSS_HAVE_EC_NIST_224| KSS_HAVE_EC_NIST_K || KSS_HAVE_EC_BP

    case kKOSE_ECCurve_NIST_P256:
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool256:
#endif
#if KSS_HAVE_EC_NIST_K
    case kKOSE_ECCurve_Secp256k1:
#endif
    {
        *privKeyLen = 32;
        *pubKeyLen  = 65;
...
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool320: {
        *privKeyLen = 40;
        *pubKeyLen  = 81;
    } break;
#endif
    case kKOSE_ECCurve_NIST_P384:
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool384:
#endif
    {
        *privKeyLen = 48;
        *pubKeyLen  = 97;
...
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool512: {
        *privKeyLen = 64;
        *pubKeyLen  = 129;
    } break;
#endif
...
#if KSS_HAVE_EC_MONT
    case kKOSE_ECCurve_ECC_MONT_DH_25519:
#endif
#if KSS_HAVE_EC_ED
    case kKOSE_ECCurve_ECC_ED_25519:
#endif
    {
        *privKeyLen = 32;
        *pubKeyLen  = 32;
...
#if KSSFTR_KOSE_ECC && KSSFTR_KOSE_KEY_SET
/* kss_kose_create_curve_if_needed for internal to this file and for tests */
smStatus_t kss_kose_create_curve_if_needed(KoseSession_t *pSession, uint32_t curve_id)
{
    smStatus_t status = SM_NOT_OK;
    //uint32_t existing_curve_id = 0;
    uint8_t curveList[kKOSE_ECCurve_Total_Weierstrass_Curves] = {
        0,
    };
...
#if KSS_HAVE_EC_ED
    if (curve_id == kKOSE_ECCurve_RESERVED_ID_ECC_ED_25519) {
...
#if KSS_HAVE_EC_MONT
    if (curve_id == kKOSE_ECCurve_RESERVED_ID_ECC_MONT_DH_25519
#if KSS_HAVE_KOSE_VER_GTE_07_02
        || curve_id == kKOSE_ECCurve_RESERVED_ID_ECC_MONT_DH_448
#endif
    ) {
#if KSS_HAVE_KOSE_VER_GTE_07_02
        status = Kose_API_CreateECCurve(pSession, curve_id);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            return status;
        }
        else {
            /* If curve is already created, Kose_API_CreateECCurve fails. Ignore this error */
            return SM_OK;
        }
    ...
    status = Kose_API_ReadECCurveList(pSession, curveList, &curveListLen);
    if (status == SM_OK) {
        if (curve_id == 0) {
            return SM_NOT_OK;
        }
        ...
        if (curveList[curve_id - 1] == kKOSE_SetIndicator_SET) {
            return SM_OK;
        }
    }
    else {
...
#if KSS_HAVE_EC_NIST_192
    case kKOSE_ECCurve_NIST_P192:
        status = Kose_API_CreateCurve_prime192v1(pSession, curve_id);
        break;
#endif
#if KSS_HAVE_EC_NIST_224
    case kKOSE_ECCurve_NIST_P224:
        status = Kose_API_CreateCurve_secp224r1(pSession, curve_id);
        break;
#endif
    case kKOSE_ECCurve_NIST_P256:
        status = Kose_API_CreateCurve_prime256v1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_NIST_P384:
        status = Kose_API_CreateCurve_secp384r1(pSession, curve_id);
        break;
#if KSS_HAVE_EC_NIST_521
    case kKOSE_ECCurve_NIST_P521:
        status = Kose_API_CreateCurve_secp521r1(pSession, curve_id);
        break;
#endif
#if KSS_HAVE_EC_BP
    case kKOSE_ECCurve_Brainpool160:
        status = Kose_API_CreateCurve_brainpoolP160r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool192:
        status = Kose_API_CreateCurve_brainpoolP192r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool224:
        status = Kose_API_CreateCurve_brainpoolP224r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool256:
        status = Kose_API_CreateCurve_brainpoolP256r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool320:
        status = Kose_API_CreateCurve_brainpoolP320r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool384:
        status = Kose_API_CreateCurve_brainpoolP384r1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Brainpool512:
        status = Kose_API_CreateCurve_brainpoolP512r1(pSession, curve_id);
        break;
#endif
#if KSS_HAVE_EC_NIST_K
    case kKOSE_ECCurve_Secp160k1:
        status = Kose_API_CreateCurve_secp160k1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Secp192k1:
        status = Kose_API_CreateCurve_secp192k1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Secp224k1:
        status = Kose_API_CreateCurve_secp224k1(pSession, curve_id);
        break;
    case kKOSE_ECCurve_Secp256k1:
        status = Kose_API_CreateCurve_secp256k1(pSession, curve_id);
        break;
#endif
    default:
        break;
...
static uint8_t CheckIfKeyIdExists(uint32_t keyId, pKoseSession_t session_ctx, smStatus_t *apduStatus)
{
    ...
    KOSE_Result_t IdExists = kKOSE_Result_NA;

    retStatus = Kose_API_CheckObjectExists(session_ctx, keyId, &IdExists);
    if (apduStatus != NULL) {
        *apduStatus = retStatus;
    }
    if (retStatus == SM_OK) {
        if (IdExists == kKOSE_Result_SUCCESS) {
        ...
        LOG_E("Error in Kose_API_CheckObjectExists");
        return 0;
    }
}
#endif
    ...
    size_t policy_buff_len)
{
...
#endif // KSS_HAVE_EC_MONT || KSS_HAVE_EC_ED
    {
        asn_retval = kss_util_pkcs8_asn1_get_ec_pair_key_index(
            key, keyLen, &publicKeyIndex, &publicKeyLen, &privateKeyIndex, &privateKeyLen);
        if (asn_retval != kStatus_KSS_Success) {
            LOG_W("error in kss_util_pkcs8_asn1_get_ec_pair_key_index");
            goto exit;
        }
    }
    ...
    size_t policy_buff_len)
{
    ...
    if (keyObject->objectType == kKSS_KeyPart_Pair) {
        ...
        if (kssStatus != kStatus_KSS_Success) {
            LOG_E("Error in kss_kose_key_store_set_ecc_keypair");
            retval = kssStatus;
            goto exit;
        }
    }
...
static kss_status_t kss_kose_key_store_set_aes_key(kss_kose_key_store_t *keyStore,
    kss_kose_object_t *keyObject,
    const uint8_t *key,
    size_t keyLen,
    size_t keyBitLen,
    ...
    size_t policy_buff_len)
{
    ...
    KosePolicy_t kose_policy;
    KOSE_INS_t transient_type;
    KOSE_SymmKeyType_t type = kKOSE_SymmKeyType_NA;
    KOSE_KeyID_t kekID      = KOSE_KeyID_KEK_NONE;
    uint8_t IdExists         = 0;
    KOSE_Result_t objExists = kKOSE_Result_NA;
    smStatus_t apduRetValue  = SM_NOT_OK;
    ...
    objExists = (IdExists == 1) ? kKOSE_Result_SUCCESS : kKOSE_Result_FAILURE;

    kose_policy.value     = (uint8_t *)policy_buff;
    kose_policy.value_len = policy_buff_len;

    if (keyBitLen % 8 == 0) {
        if (keyObject->cipherType == kKSS_CipherType_AES) {
            type = kKOSE_SymmKeyType_AES;
        }
        else if (keyObject->cipherType == kKSS_CipherType_CMAC) {
            type = kKOSE_SymmKeyType_CMAC;
        }
        else if (keyObject->cipherType == kKSS_CipherType_HMAC) {
            type = kKOSE_SymmKeyType_HMAC;
        }

        if (keyStore->kekKey != NULL) {
            kekID = keyStore->kekKey->keyId;
        }
        status = kss_kose_LL_set_symm_key(&keyStore->session->s_ctx,
            &kose_policy,
            KOSE_MaxAttemps_NA,
            keyObject->keyId,
            kekID,
...
#if KSSFTR_KOSE_KEY_SET
static kss_status_t kss_kose_key_store_set_des_key(kss_kose_key_store_t *keyStore,
    kss_kose_object_t *keyObject,
    const uint8_t *key,
    size_t keyLen,
    size_t keyBitLen,
...
#if 0
static kss_status_t kss_kose_key_store_set_deswrapped_key(
    ...
    size_t policy_buff_len)
{
    ...
    if (keyObject->isPersistant) {
        status = Kose_API_DES_SetNewWrapped_P(&keyStore->session->s_ctx,
            &kose_policy,
            keyObject->keyId,
            keyObject->kekId,
            (U16)keyBitLen,
        ...
        status = Kose_API_DES_SetNewWrapped_T(&keyStore->session->s_ctx,
            &kose_policy,
            keyObject->keyId,
            keyObject->kekId,
            (U16)keyBitLen,
...
#if KSSFTR_KOSE_KEY_SET
static kss_status_t kss_kose_key_store_set_cert(kss_kose_key_store_t *keyStore,
    kss_kose_object_t *keyObject,
    const uint8_t *key,
    size_t keyLen,
    size_t keyBitLen,
    ...
    size_t policy_buff_len)
{
    ...
    KosePolicy_t kose_policy;
#if !KSS_HAVE_KOSE_VER_GTE_07_02
    KosePolicy_t *ppolicy = &kose_policy;
#endif
    uint16_t data_rem;
    uint16_t offset   = 0;
    uint16_t fileSize = 0;
    ...
    while (data_rem > 0) {
        ...
        /* Call APIs For SE051 */
        obj_exists = (IdExists == 1) ? kKOSE_Result_SUCCESS : kKOSE_Result_FAILURE;
        if (obj_exists == kKOSE_Result_FAILURE && offset == 0) {
            status = Kose_API_WriteBinary_Ver(&keyStore->session->s_ctx,
                &kose_policy,
                keyObject->keyId,
                offset,
                (uint16_t)fileSize,
            ...
            status = Kose_API_UpdateBinary_Ver(&keyStore->session->s_ctx,
                &kose_policy,
                keyObject->keyId,
                offset,
                (uint16_t)fileSize,
...
#if 0
static kss_status_t kss_kose_key_store_set_pcr(
    kss_kose_key_store_t *keyStore,
    kss_kose_object_t *keyObject,
    const uint8_t *key,
    size_t keyLen,
    ...
    size_t policy_buff_len)
{
    ...
    if (keyObject->cipherType == kKSS_CipherType_PCR) {
        status = Kose_API_WritePCR_WithType(&keyStore->session->s_ctx,
            kKOSE_INS_NA,
            &kose_policy,
            keyObject->keyId,
            key,
    ...
    else if (keyObject->cipherType == kKSS_CipherType_Update_PCR) {
        status = Kose_API_WritePCR_WithType(&keyStore->session->s_ctx,
            kKOSE_INS_NA,
            &kose_policy,
            keyObject->keyId,
    ...
    else if (keyObject->cipherType == kKSS_CipherType_Reset_PCR) {
        status = Kose_API_WritePCR_WithType(&keyStore->session->s_ctx,
            kKOSE_INS_NA,
            &kose_policy,
            keyObject->keyId,
    ...
    kss_kose_key_store_t *keyStore, kss_kose_object_t *keyObject, size_t keyBitLen, void *options)
{
    ...
    ENSURE_OR_GO_EXIT(keyObject->objectType == kKSS_KeyPart_Pair);
...
#endif
    {
        ...
        if (keyObject->curve_id == kKOSE_ECCurve_NA) {
            keyObject->curve_id =
                (KOSE_ECCurve_t)kose_kssKeyTypeLenToCurveId((kss_cipher_type_t)keyObject->cipherType, keyBitLen);
        }
        ...
        curve_id = (IdExists == 1) ? kKOSE_ECCurve_NA : (KOSE_ECCurve_t)keyObject->curve_id;

        status = Kose_API_WriteECKey(&keyStore->session->s_ctx,
            &kose_policy,
            KOSE_MaxAttemps_NA,
            keyObject->keyId,
            curve_id,
    ...
    case kKSS_CipherType_RSA_CRT: {
        ...
        KOSE_KeyPart_t key_part = kKOSE_KeyPart_Pair;
        KOSE_RSAKeyFormat_t rsa_format;
        size_t keyBitLength = 0;
        if (keyObject->cipherType == kKSS_CipherType_RSA) {
            rsa_format = kKOSE_RSAKeyFormat_RAW;
...
{
    kss_status_t retval           = kStatus_KSS_Fail;
    kss_cipher_type_t cipher_type = (kss_cipher_type_t)keyObject->cipherType;
    smStatus_t status             = SM_NOT_OK;
    uint16_t size;
    uint32_t attestID;
    KOSE_AttestationAlgo_t attestAlgo;
...
#endif
    {
        KOSE_ECSignatureAlgo_t ecSignAlgo = (KOSE_ECSignatureAlgo_t)kose_get_ec_sign_hash_mode(algorithm_attst);
        attestAlgo                         = (KOSE_AttestationAlgo_t)ecSignAlgo;
    } break;
    ...
    case kKSS_CipherType_RSA:
    case kKSS_CipherType_RSA_CRT: {
        KOSE_RSASignatureAlgo_t rsaSigningAlgo = kose_get_rsa_sign_hash_mode(algorithm_attst);
        attestAlgo                              = (KOSE_AttestationAlgo_t)rsaSigningAlgo;
    } break;
#endif
    default:
        goto exit;
...
#endif
    {
        ...
        attst_data->data[0].timeStampLen = sizeof(KOSE_TimeStamp_t);
#if KSS_HAVE_KOSE_VER_GTE_07_02
        status = Kose_API_ReadObject_W_Attst_V2(&keyStore->session->s_ctx,
            keyObject->keyId,
            0,
            0,
            attestID,
...
#else
        status = Kose_API_ReadObject_W_Attst(&keyStore->session->s_ctx,
            keyObject->keyId,
            0,
            0,
            attestID,
    ...
    case kKSS_CipherType_RSA_CRT: {
        ...
        if (attestAlgo == kKOSE_AttestationAlgo_RSA_SHA_512_PKCS1 ||
            attestAlgo == kKOSE_AttestationAlgo_RSA_SHA512_PKCS1_PSS) {
            status = Kose_API_ReadSize(&keyStore->session->s_ctx, keyObject_attst->keyId, &key_size_bytes);
            if (status == SM_ERR_APDU_THROUGHPUT) {
                return kStatus_KSS_ApduThroughputError;
            }
            if (status != SM_OK) {
        ...
        attst_data->data[0].timeStampLen = sizeof(KOSE_TimeStamp_t);
#if KSS_HAVE_KOSE_VER_GTE_07_02
        status = Kose_API_ReadRSA_W_Attst_V2(&keyStore->session->s_ctx,
            keyObject->keyId,
            0,
            0,
            kKOSE_RSAPubKeyComp_MOD,
            attestID,
            attestAlgo,
            random_attst,
            randomLen_attst,
            ...
            &(attst_data->data[0].signatureLen));
#else
        status = Kose_API_ReadRSA_W_Attst(&keyStore->session->s_ctx,
            keyObject->keyId,
            0,
            0,
            kKOSE_RSAPubKeyComp_MOD,
        ...
        attst_data->data[1].timeStampLen = sizeof(KOSE_TimeStamp_t);
#if KSS_HAVE_KOSE_VER_GTE_07_02
        status = Kose_API_ReadRSA_W_Attst_V2(&keyStore->session->s_ctx,
            keyObject->keyId,
            0,
            0,
            kKOSE_RSAPubKeyComp_PUB_EXP,
            attestID,
            attestAlgo,
            random_attst,
            randomLen_attst,
...
kss_status_t kss_kose_key_store_get_key_fromoffset(kss_kose_key_store_t *keyStore,
    kss_kose_object_t *keyObject,
    uint8_t *key,
    size_t *keylen,
    size_t *pKeyBitLen,
...
{
    kss_status_t retval = kStatus_KSS_Fail;
    kss_key_type_t key_type = keyObject->objectType;
    smStatus_t status = SM_NOT_OK;

    switch (key_type) {
    case kKSS_KeyType_Certificate:
        status =
            Kose_API_FIL_BinaryReadFromOffset(&keyStore->session->s_ctx,
                keyObject->keyId,
                (uint16_t)*keylen,
                offset,
                key,
    ...
    kss_kose_key_store_t *keyStore, kss_kose_object_t *keyObject, uint8_t *key, size_t *keylen)
{
    ...
    case kKSS_CipherType_DES: {
        status =
            Kose_API_ExportObject(&keyStore->session->s_ctx, keyObject->keyId, kKOSE_RSAKeyComponent_NA, key, keylen);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
            goto exit;
        }
    ...
    kss_kose_key_store_t *keyStore, kss_kose_object_t *keyObject, uint8_t *key, size_t keylen)
{
    ...
    case kKSS_CipherType_DES: {
        status =
            Kose_API_ImportObject(&keyStore->session->s_ctx, keyObject->keyId, kKOSE_RSAKeyComponent_NA, key, keylen);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
            goto exit;
        }
    ...
    size_t dataLen)
{
    kss_status_t retval           = kStatus_KSS_Fail;
    smStatus_t status             = SM_NOT_OK;
    KOSE_CipherMode_t cipherMode = kose_get_cipher_mode(context->algorithm);
    KOSE_Cipher_Oper_OneShot_t OperType =
        (context->mode == kMode_KSS_Encrypt) ? kKOSE_Cipher_Oper_OneShot_Encrypt : kKOSE_Cipher_Oper_OneShot_Decrypt;
    size_t blockLenModulus = dataLen % CIPHER_BLOCK_SIZE;

    ENSURE_OR_GO_EXIT(cipherMode != kKOSE_CipherMode_NA);
    ENSURE_OR_GO_EXIT(
        !((context->algorithm == kAlgorithm_KSS_AES_CTR_INT_IV) && (OperType == kKOSE_Cipher_Oper_OneShot_Decrypt)));

    if ((context->algorithm == kAlgorithm_KSS_AES_CTR) || (context->algorithm == kAlgorithm_KSS_AES_CTR_INT_IV)) {
        ENSURE_OR_GO_EXIT(blockLenModulus == 0);
    }

    status = Kose_API_CipherOneShot(&context->session->s_ctx,
        context->keyObject->keyId,
        cipherMode,
        srcData,
        dataLen,
...
kss_status_t kss_kose_cipher_init(kss_kose_symmetric_t *context, uint8_t *iv, size_t ivLen)
{
    ...
    smStatus_t status;
    //size_t retdataLen = 0;
    KOSE_Cipher_Oper_t OperType =
        (context->mode == kMode_KSS_Encrypt) ? kKOSE_Cipher_Oper_Encrypt : kKOSE_Cipher_Oper_Decrypt;
    KOSE_CipherMode_t cipherMode = kose_get_cipher_mode(context->algorithm);

#if KSSFTR_KOSE_CREATE_DELETE_CRYPTOOBJ
    KOSE_CryptoModeSubType_t subtype;
    uint8_t list[1024] = {
        0,
    };
    ...
    ENSURE_OR_GO_EXIT(cipherMode != kKOSE_CipherMode_NA);
    ENSURE_OR_GO_EXIT(
        !((context->algorithm == kAlgorithm_KSS_AES_CTR_INT_IV) && (OperType == kKOSE_Cipher_Oper_Decrypt)))

    subtype.cipher = (cipherMode == kKOSE_CipherMode_AES_CTR_INT_IV) ? kKOSE_CipherMode_AES_CTR : cipherMode;
    switch (context->algorithm) {
    case kAlgorithm_KSS_AES_ECB:
        context->cryptoObjectId = kKOSE_CryptoObject_AES_ECB_NOPAD;
        break;
    case kAlgorithm_KSS_AES_CBC:
        context->cryptoObjectId = kKOSE_CryptoObject_AES_CBC_NOPAD;
        break;
    case kAlgorithm_KSS_AES_CTR:
        context->cryptoObjectId = kKOSE_CryptoObject_AES_CTR;
        break;
    case kAlgorithm_KSS_AES_CTR_INT_IV:
        context->cryptoObjectId = kKOSE_CryptoObject_AES_CTR_INT_IV;
        break;
    case kAlgorithm_KSS_DES_ECB:
    case kAlgorithm_KSS_DES3_ECB:
        context->cryptoObjectId = kKOSE_CryptoObject_DES_ECB_NOPAD;
        break;
    case kAlgorithm_KSS_DES_CBC:
    case kAlgorithm_KSS_DES3_CBC:
        context->cryptoObjectId = kKOSE_CryptoObject_DES_CBC_NOPAD;
        break;
    case kAlgorithm_KSS_DES_CBC_ISO9797_M1:
    case kAlgorithm_KSS_DES3_CBC_ISO9797_M1:
        context->cryptoObjectId = kKOSE_CryptoObject_DES_CBC_ISO9797_M1;
        break;
    case kAlgorithm_KSS_DES_CBC_ISO9797_M2:
    case kAlgorithm_KSS_DES3_CBC_ISO9797_M2:
        context->cryptoObjectId = kKOSE_CryptoObject_DES_CBC_ISO9797_M2;
        break;
    default:
        return kStatus_KSS_Fail;
    }

    status = Kose_API_ReadCryptoObjectList(&context->session->s_ctx, list, &listlen);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        return kStatus_KSS_ApduThroughputError;
    }
    ENSURE_OR_GO_EXIT(status == SM_OK);
    ...
    if (create_crypto_obj) {
        status = Kose_API_CreateCryptoObject(
            &context->session->s_ctx, context->cryptoObjectId, kKOSE_CryptoContext_CIPHER, subtype);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            return kStatus_KSS_ApduThroughputError;
        }
        if (status != SM_OK) {
    ...
    if (cipherMode == kKOSE_CipherMode_AES_ECB_NOPAD || cipherMode == kKOSE_CipherMode_DES_ECB_NOPAD ||
        cipherMode == kKOSE_CipherMode_DES_ECB_ISO9797_M1 || cipherMode == kKOSE_CipherMode_DES_ECB_ISO9797_M2) {
        ivLen = 0;
    }

    status = Kose_API_CipherInit(
    ...
    kss_kose_symmetric_t *context, const uint8_t *srcData, size_t srcLen, uint8_t *destData, size_t *destLen)
{
    ...
    if (context->algorithm == kAlgorithm_KSS_DES_ECB || context->algorithm == kAlgorithm_KSS_DES_CBC ||
        context->algorithm == kAlgorithm_KSS_DES3_ECB || context->algorithm == kAlgorithm_KSS_DES3_CBC) {
    ...
    if ((context->cache_data_len + srcLen) < cipherBlockSize) {
        ...
        *destLen                = 0;
        ...
        if (context->cache_data_len > 0) {
            ...
            status      = Kose_API_CipherUpdate(&context->session->s_ctx,
    ...
    kss_kose_symmetric_t *context, const uint8_t *srcData, size_t srcLen, uint8_t *destData, size_t *destLen)
{
    ...
    if (context->algorithm == kAlgorithm_KSS_AES_ECB || context->algorithm == kAlgorithm_KSS_AES_CBC) {
...
void kss_kose_symmetric_context_free(kss_kose_symmetric_t *context)
{
    ...
    if (context->cryptoObjectId != 0) {
        ...
        if (object_exists) {
            status = Kose_API_DeleteCryptoObject(&context->session->s_ctx, context->cryptoObjectId);
            if (status != SM_OK) {
    ...
    kss_mode_t mode)
{
    ...
    context->session    = session;
    context->keyObject  = keyObject;
    if ((algorithm == kAlgorithm_KSS_AES_CCM) || (algorithm == kAlgorithm_KSS_AES_GCM) ||
        (algorithm == kAlgorithm_KSS_AES_GCM_INT_IV) || (algorithm == kAlgorithm_KSS_AES_CCM_INT_IV)) {
        context->algorithm = algorithm;
    }
    else {
    ...
    size_t *tagLen)
{
    ...
    KOSE_CipherMode_t cipherMode =
        (context->algorithm == kAlgorithm_KSS_AES_GCM) ? kKOSE_CipherMode_AES_GCM : kKOSE_CipherMode_AES_GCM_INT_IV;
    KOSE_Cipher_Oper_OneShot_t OperType =
        (context->mode == kMode_KSS_Encrypt) ? kKOSE_Cipher_Oper_OneShot_Encrypt : kKOSE_Cipher_Oper_OneShot_Decrypt;

    // Not support decrypt with internal IV.
    ENSURE_OR_GO_EXIT(
        !((context->algorithm == kAlgorithm_KSS_AES_GCM_INT_IV) && (OperType == kKOSE_Cipher_Oper_OneShot_Decrypt)));

    ENSURE_OR_GO_EXIT(context->keyObject != NULL);
    status = Kose_API_AeadOneShot(&context->session->s_ctx,
        context->keyObject->keyId,
        cipherMode,
        srcData,
        size,
    ...
    kss_kose_aead_t *context, uint8_t *nonce, size_t nonceLen, size_t tagLen, size_t aadLen, size_t payloadLen)
{
...
#if KSS_HAVE_KOSE_VER_GTE_07_02
    smStatus_t status             = SM_NOT_OK;
    KOSE_CipherMode_t cipherMode = kKOSE_CipherMode_NA;
    KOSE_Cipher_Oper_t OperType =
        (context->mode == kMode_KSS_Encrypt) ? kKOSE_Cipher_Oper_Encrypt : kKOSE_Cipher_Oper_Decrypt;
#if KSSFTR_KOSE_CREATE_DELETE_CRYPTOOBJ
    KOSE_CryptoModeSubType_t subtype;
    ...
    // Not support decrypt with internal IV.
    ENSURE_OR_GO_EXIT(
        !((context->algorithm == kAlgorithm_KSS_AES_GCM_INT_IV) && (OperType == kKOSE_Cipher_Oper_Decrypt)));
    ENSURE_OR_GO_EXIT(
        !((context->algorithm == kAlgorithm_KSS_AES_CCM_INT_IV) && (OperType == kKOSE_Cipher_Oper_Decrypt)));

    if (context->algorithm == kAlgorithm_KSS_AES_GCM) {
        context->cryptoObjectId = kKOSE_CryptoObject_AES_GCM;
        subtype.aead            = kKOSE_AeadGCMAlgo;
    }
    else if (context->algorithm == kAlgorithm_KSS_AES_GCM_INT_IV) {
        context->cryptoObjectId = kKOSE_CryptoObject_AES_GCM_INT_IV;
#if KSS_HAVE_KOSE_VER_GTE_07_02
        subtype.aead = kKOSE_AeadGCMAlgo;
#else
        subtype.aead = kKOSE_AeadGCM_IVAlgo;
#endif
    }
    else if (context->algorithm == kAlgorithm_KSS_AES_CCM) {
        context->cryptoObjectId = kKOSE_CryptoObject_AES_CCM;
        subtype.aead            = kKOSE_AeadCCMAlgo;
    }
    else if (context->algorithm == kAlgorithm_KSS_AES_CCM_INT_IV) {
        context->cryptoObjectId = kKOSE_CryptoObject_AES_CCM_INT_IV;
        subtype.aead            = kKOSE_AeadCCMAlgo;
    }
    else {
        goto exit;
    ...
    if (create_crypto_obj) {
        status = Kose_API_CreateCryptoObject(
            &context->session->s_ctx, context->cryptoObjectId, kKOSE_CryptoContext_AEAD, subtype);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            return kStatus_KSS_ApduThroughputError;
        }
        if (status != SM_OK) {
    ...
    if ((context->algorithm == (kAlgorithm_KSS_AES_GCM)) || (context->algorithm == (kAlgorithm_KSS_AES_GCM_INT_IV))) {
        cipherMode = (context->algorithm == kAlgorithm_KSS_AES_GCM) ? kKOSE_CipherMode_AES_GCM :
                                                                      kKOSE_CipherMode_AES_GCM_INT_IV;
        status     = Kose_API_AeadInit(&context->session->s_ctx,
            context->keyObject->keyId,
            cipherMode,
            context->cryptoObjectId,
            nonce,
        ...
        cipherMode = (context->algorithm == kAlgorithm_KSS_AES_CCM) ? kKOSE_CipherMode_AES_CCM :
                                                                      kKOSE_CipherMode_AES_CCM_INT_IV;
        status     = Kose_API_AeadCCMInit(&context->session->s_ctx,
            context->keyObject->keyId,
            cipherMode,
            context->cryptoObjectId,
            nonce,
...
kss_status_t kss_kose_aead_update_aad(kss_kose_aead_t *context, const uint8_t *aadData, size_t aadDataLen)
{
    ...
    if (aadDataLen > AEAD_BLOCK_SIZE) {
        while ((aadDataLen - src_offset) >= AEAD_BLOCK_SIZE) {
            ...
            status = Kose_API_AeadUpdate_aad(
        ...
        status = Kose_API_AeadUpdate_aad(&context->session->s_ctx, context->cryptoObjectId, aadData, aadDataLen);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
            goto exit;
        }
    ...
    if ((context->cache_data_len + srcLen) < AEAD_BLOCK_SIZE) {
        ...
        *destLen                = 0;
        ...
        if (context->cache_data_len > 0) {
            ...
            blockoutLen = outBuffSize;
            status      = Kose_API_AeadUpdate(&context->session->s_ctx,
                context->cryptoObjectId,
                context->cache_data,
                CIPHER_BLOCK_SIZE,
                destData,
    ...
    size_t *tagLen)
{
    ...
    if ((context->algorithm == kAlgorithm_KSS_AES_CCM) || (context->algorithm == kAlgorithm_KSS_AES_CCM_INT_IV)) {
        retval = kss_kose_aead_CCMfinish(context, srcData, srcLen, destData, destLen, tag, tagLen);
    }
    else {
        if (context->cache_data_len != 0) {
            memcpy(srcdata_updated, context->cache_data, context->cache_data_len);
        ...
        status = Kose_API_AeadFinal(&context->session->s_ctx, context->cryptoObjectId, tag, tagLen, OperType);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
            goto exit;
        }
    ...
    size_t *tagLen)
{
    ...
    if (srcdata_updated_len > 0) {
        if (srcdata_updated_len < CIPHER_BLOCK_SIZE) {
            status = Kose_API_AeadCCMLastUpdate(
            ...
            if (srcdata_updated_len) {
                status = Kose_API_AeadCCMLastUpdate(&context->session->s_ctx,
                    context->cryptoObjectId,
...
#endif /* KSSFTR_KOSE_CREATE_DELETE_CRYPTOOBJ */
    memset(context, 0, sizeof(*context));
#else
    AX_UNUSED_ARG(context);
#endif /* KSS_HAVE_KOSE_VER_GTE_07_02 */
    ...
    kss_kose_mac_t *context, const uint8_t *message, size_t messageLen, uint8_t *mac, size_t *macLen)
{
    kss_status_t retval          = kStatus_KSS_Fail;
    smStatus_t status            = SM_NOT_OK;
    KOSE_MACAlgo_t macOperation = kose_get_mac_algo(context->algorithm);

    ENSURE_OR_GO_EXIT(message != NULL);
    ENSURE_OR_GO_EXIT(mac != NULL);
    ENSURE_OR_GO_EXIT(macLen != NULL);

    ENSURE_OR_GO_EXIT(macOperation != kKOSE_MACAlgo_NA);

    if (context->mode == kMode_KSS_Mac) {
        status = Kose_API_MACOneShot_G(
    ...
    else if (context->mode == kMode_KSS_Mac_Validate) {
        KOSE_Result_t result = kKOSE_Result_FAILURE;
        size_t result_size    = sizeof(result);

        status = Kose_API_MACOneShot_V(&context->session->s_ctx,
            context->keyObject->keyId,
            macOperation,
            message,
            messageLen,
        ...
        ENSURE_OR_GO_EXIT(result == kKOSE_Result_SUCCESS);
    }
    else {
        LOG_E("Unknown mode");
        goto exit;
    ...
    kss_kose_mac_t *context, const uint8_t *message, size_t messageLen, uint8_t *mac, size_t macLen)
{
    ...
    macOperation = kose_get_mac_algo(context->algorithm);
...
kss_status_t kss_kose_mac_init(kss_kose_mac_t *context)
{
    kss_status_t retval       = kStatus_KSS_Fail;
    smStatus_t status         = SM_NOT_OK;
    KOSE_Mac_Oper_t operType = kKOSE_Mac_Oper_NA;
#if KSSFTR_KOSE_CREATE_DELETE_CRYPTOOBJ
    KOSE_CryptoModeSubType_t subtype;
    ...
    KOSE_CryptoContext_t cryptoContext;

    switch (context->algorithm) {
    case kAlgorithm_KSS_CMAC_AES:
        subtype.mac             = kKOSE_MACAlgo_CMAC_128;
        cryptoContext           = kKOSE_CryptoContext_SIGNATURE;
        context->cryptoObjectId = kKOSE_CryptoObject_CMAC_128;
        break;
#if KSS_HAVE_HASH_1
    case kAlgorithm_KSS_HMAC_SHA1:
        subtype.mac             = kKOSE_MACAlgo_HMAC_SHA1;
        cryptoContext           = kKOSE_CryptoContext_SIGNATURE;
        context->cryptoObjectId = kKOSE_CryptoObject_HMAC_SHA1;
        break;
#endif
    case kAlgorithm_KSS_HMAC_SHA256:
        subtype.mac             = kKOSE_MACAlgo_HMAC_SHA256;
        cryptoContext           = kKOSE_CryptoContext_SIGNATURE;
        context->cryptoObjectId = kKOSE_CryptoObject_HMAC_SHA256;
        break;
    case kAlgorithm_KSS_HMAC_SHA384:
        subtype.mac             = kKOSE_MACAlgo_HMAC_SHA384;
        cryptoContext           = kKOSE_CryptoContext_SIGNATURE;
        context->cryptoObjectId = kKOSE_CryptoObject_HMAC_SHA384;
        break;
#if KSS_HAVE_HASH_512
    case kAlgorithm_KSS_HMAC_SHA512:
        subtype.mac             = kKOSE_MACAlgo_HMAC_SHA512;
        cryptoContext           = kKOSE_CryptoContext_SIGNATURE;
        context->cryptoObjectId = kKOSE_CryptoObject_HMAC_SHA512;
        break;
#endif
    default:
        return kStatus_KSS_Fail;
    ...
    if (create_crypto_obj) {
        status =
            Kose_API_CreateCryptoObject(&context->session->s_ctx, context->cryptoObjectId, cryptoContext, subtype);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            return kStatus_KSS_ApduThroughputError;
        }
        if (status != SM_OK) {
    ...
    if (context->mode == kMode_KSS_Mac) {
        operType = kKOSE_Mac_Oper_Generate;
    }
    else if (context->mode == kMode_KSS_Mac_Validate) {
        operType = kKOSE_Mac_Oper_Validate;
    }
    else {
        LOG_E("Unknown mode");
        goto exit;
    ...
    status = Kose_API_MACInit(&context->session->s_ctx, context->keyObject->keyId, context->cryptoObjectId, operType);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        return kStatus_KSS_ApduThroughputError;
    }
    ENSURE_OR_GO_EXIT(status == SM_OK);
...
kss_status_t kss_kose_mac_update(kss_kose_mac_t *context, const uint8_t *message, size_t messageLen)
{
    ...
    status = Kose_API_MACUpdate(&context->session->s_ctx, message, messageLen, context->cryptoObjectId);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        retval = kStatus_KSS_ApduThroughputError;
        goto exit;
    }
...
kss_status_t kss_kose_mac_finish(kss_kose_mac_t *context, uint8_t *mac, size_t *macLen)
{
    ...
    if (context->mode == kMode_KSS_Mac) {
        status = Kose_API_MACFinal(&context->session->s_ctx, NULL, 0, context->cryptoObjectId, NULL, 0, mac, macLen);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            retval = kStatus_KSS_ApduThroughputError;
            goto exit;
        }
...
{
    if (context->cryptoObjectId != 0) {
        smStatus_t status = Kose_API_DeleteCryptoObject(&context->session->s_ctx, context->cryptoObjectId);
        if (status != SM_OK) {
            LOG_D("Could not delete crypto object 0x04X", context->cryptoObjectId);
            return;
        }
    ...
    kss_kose_digest_t *context, const uint8_t *message, size_t messageLen, uint8_t *digest, size_t *digestLen)
{
    kss_status_t retval = kStatus_KSS_Fail;
    smStatus_t status   = SM_NOT_OK;
    uint8_t sha_type    = kose_get_sha_algo(context->algorithm);

    ENSURE_OR_GO_EXIT(sha_type != kKOSE_DigestMode_NA);

    status = Kose_API_SHAOneShot(&context->session->s_ctx, sha_type, message, messageLen, digest, digestLen);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        retval     = kStatus_KSS_ApduThroughputError;
        *digestLen = 0;
        goto exit;
...
kss_status_t kss_kose_digest_init(kss_kose_digest_t *context)
{
    ...
    case kAlgorithm_KSS_SHA1:
        subtype.digest          = kKOSE_DigestMode_SHA;
        context->cryptoObjectId = kKOSE_CryptoObject_DIGEST_SHA;
        break;
#endif
#if KSS_HAVE_HASH_224
    case kAlgorithm_KSS_SHA224:
        subtype.digest          = kKOSE_DigestMode_SHA224;
        context->cryptoObjectId = kKOSE_CryptoObject_DIGEST_SHA224;
        break;
#endif
    case kAlgorithm_KSS_SHA256:
        subtype.digest          = kKOSE_DigestMode_SHA256;
        context->cryptoObjectId = kKOSE_CryptoObject_DIGEST_SHA256;
        break;
    case kAlgorithm_KSS_SHA384:
        subtype.digest          = kKOSE_DigestMode_SHA384;
        context->cryptoObjectId = kKOSE_CryptoObject_DIGEST_SHA384;
        break;
#if KSS_HAVE_HASH_512
    case kAlgorithm_KSS_SHA512:
        subtype.digest          = kKOSE_DigestMode_SHA512;
        context->cryptoObjectId = kKOSE_CryptoObject_DIGEST_SHA512;
        break;
#endif
    default:
        return kStatus_KSS_Fail;
    ...
    if (create_crypto_obj) {
        status = Kose_API_CreateCryptoObject(
            &context->session->s_ctx, context->cryptoObjectId, kKOSE_CryptoContext_DIGEST, subtype);
        if (status == SM_ERR_APDU_THROUGHPUT) {
            return kStatus_KSS_ApduThroughputError;
        }
        if (status != SM_OK) {
    ...
    status = Kose_API_DigestInit(&context->session->s_ctx, context->cryptoObjectId);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        retval = kStatus_KSS_ApduThroughputError;
        goto exit;
    }
...
kss_status_t kss_kose_digest_update(kss_kose_digest_t *context, const uint8_t *message, size_t messageLen)
{
    ...
    status = Kose_API_DigestUpdate(&context->session->s_ctx, context->cryptoObjectId, message, messageLen);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        retval = kStatus_KSS_ApduThroughputError;
        goto exit;
    }
...
kss_status_t kss_kose_digest_finish(kss_kose_digest_t *context, uint8_t *digest, size_t *digestLen)
{
    ...
    status = Kose_API_DigestFinal(&context->session->s_ctx, context->cryptoObjectId, NULL, 0, digest, digestLen);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        retval = kStatus_KSS_ApduThroughputError;
        goto exit;
    }
...
#ifdef KSS_USE_SCP03_THREAD_SAFETY
#if KSS_HAVE_SCP_SCP03_KSS && USE_LOCK
        if ((pSession->authType == kKSS_AuthType_SCP03) && (pSession->fp_Transform == &kose_Transform_scp)) {
            if (pSession->scp03_lock_init) {
                LOCK_TXN(pSession->scp03_lock);
                LOG_D("SCP03 Channel Locked");
            }
        }
#endif // KSS_HAVE_SCP_SCP03_KSS && USE_LOCK
#endif //#ifdef KSS_USE_SCP03_THREAD_SAFETY
...
static smStatus_t kss_kose_channel_txnRaw(void *conn_ctx,
    ...
    uint8_t hasle)
{
    uint8_t txBuf[KOSE_MAX_BUF_SIZE_CMD] = {0};
    size_t i                              = 0;
    ...
    uint8_t hasle)
{
    ...
    if ((pChannelCtx != NULL)) {
#if KSSFTR_KOSE_AuthECKey || KSSFTR_KOSE_AuthSession
        struct KoseSession *koseCtx = (struct KoseSession *)&pChannelCtx->kose_session->s_ctx;
        if (koseCtx->authType == kKSS_AuthType_SCP03) {
#if USE_LOCK
            LOCK_TXN(pChannelCtx->channelLock);
#endif
            retStatus = koseCtx->fp_TXn(koseCtx, hdr, cmdBuf, cmdBufLen, rsp, rspLen, hasle);
        ...
        else if (koseCtx->authType == kKSS_AuthType_None) {
#if USE_LOCK
            LOCK_TXN(pChannelCtx->channelLock);
#endif
            retStatus = koseCtx->fp_TXn(koseCtx, hdr, cmdBuf, cmdBufLen, rsp, rspLen, hasle);
        ...
        if (currAuth == kKSS_AuthType_SCP03) {
            uint32_t u32rspLen = 0;
            if ((*rspLen) > UINT32_MAX) {
                retStatus = SM_NOT_OK;
                goto exit;
            ...
            *rspLen = u32rspLen;
            ...
            retStatus = kss_kose_channel_txnRaw(conn_ctx, hdr, cmdBuf, cmdBufLen, rsp, rspLen, hasle);
            ENSURE_OR_GO_EXIT(retStatus == SM_OK);
        }
    }
...
kss_status_t kss_kose_key_store_create_curve(KoseSession_t *pSession, uint32_t curve_id)
{
    ...
    status = kss_kose_create_curve_if_needed(pSession, curve_id);
    ...
    kss_kose_session_t *session, KOSE_Applet_Feature_t feature, KOSE_Applet_Feature_Disable_t disable_features)
{
    ...
    smStatus_t status   = SM_NOT_OK;
#if KSS_HAVE_KOSE_VER_GTE_07_02
    KOSE_ExtendedFeatures_t extended = {0};
#endif
    Kose_AppletFeatures_t applet_features = {kKOSE_AppletConfig_NA, NULL};
    ...
    if (feature.AppletConfig_ECDSA_ECDH_ECDHE == 1) {
        applet_features.variant |= kKOSE_AppletConfig_ECDSA_ECDH_ECDHE;
    }
    else if (feature.AppletConfig_EDDSA == 1) {
        applet_features.variant |= kKOSE_AppletConfig_EDDSA;
    }
    else if (feature.AppletConfig_DH_MONT == 1) {
        applet_features.variant |= kKOSE_AppletConfig_DH_MONT;
    }
    else if (feature.AppletConfig_HMAC == 1) {
        applet_features.variant |= kKOSE_AppletConfig_HMAC;
    }
    else if (feature.AppletConfig_RSA_PLAIN == 1) {
        applet_features.variant |= kKOSE_AppletConfig_RSA_PLAIN;
    }
    else if (feature.AppletConfig_RSA_CRT == 1) {
        applet_features.variant |= kKOSE_AppletConfig_RSA_CRT;
    }
    else if (feature.AppletConfig_AES == 1) {
        applet_features.variant |= kKOSE_AppletConfig_AES;
    }
    else if (feature.AppletConfig_DES == 1) {
        applet_features.variant |= kKOSE_AppletConfig_DES;
    }
    else if (feature.AppletConfig_PBKDF == 1) {
        applet_features.variant |= kKOSE_AppletConfig_PBKDF;
    }
    else if (feature.AppletConfig_TLS == 1) {
        applet_features.variant |= kKOSE_AppletConfig_TLS;
    }
    else if (feature.AppletConfig_MIFARE == 1) {
        applet_features.variant |= kKOSE_AppletConfig_MIFARE;
    }
    else if (feature.AppletConfig_I2CM == 1) {
        applet_features.variant |= kKOSE_AppletConfig_I2CM;
    }
    else {
        goto exit;
    }

    status = Kose_API_SetAppletFeatures(&session->s_ctx, &applet_features);
    if (status == SM_ERR_APDU_THROUGHPUT) {
        return kStatus_KSS_ApduThroughputError;
    }
    if (status == SM_OK) {
...
#if KSSFTR_KOSE_AuthSession
static smStatus_t kose_CreateVerifyUserIDSession(
    pKoseSession_t koseSession, const uint32_t auth_id, KOSE_AuthCtx_ID_t *userId, pKosePolicy_t policy)
{
    kss_status_t retval   = kStatus_KSS_Fail;
    KOSE_Result_t exists = kKOSE_Result_FAILURE;
    smStatus_t status     = SM_NOT_OK;
    ...
    /* Check if Object exists only if auth_id is non kKOSE_AppletResID_TRANSPORT */
    /* CheckObjectExists returns 6985 KOSE if transport is Locked */
    if (auth_id == kKOSE_AppletResID_TRANSPORT) {
        status = SM_OK;
        LOG_D("Create Session with kKOSE_AppletResID_TRANSPORT");
    }
    else {
        status = Kose_API_CheckObjectExists(koseSession, auth_id, &exists);
        if (status == SM_OK && exists == kKOSE_Result_FAILURE) {
            status = SM_NOT_OK;
            LOG_E("UserID is not Provisioned!!!");
        }
    }
    if (status == SM_OK) {
        status = Kose_API_CreateSession(koseSession, auth_id, koseSession->value, &sessionIdLen);
    }
    if (status == SM_OK) {
        status = SM_NOT_OK;
        retval = kss_host_key_store_get_key(userId->pObj->keyStore, userId->pObj, keyVal, &keyValLen, &keyBitLen);
        ...
        if (retval == kStatus_KSS_Success) {
            koseSession->hasSession = 1;
            koseSession->authType   = kKSS_AuthType_ID;
            koseSession->auth_id    = auth_id;
            status                   = Kose_API_VerifySessionUserID(koseSession, keyVal, keyValLen);
            if (status == SM_OK) {
                if ((policy != NULL) && (policy->value != NULL) && (policy->value_len > 0)) {
                    status = SM_NOT_OK;
                    status = Kose_API_ExchangeSessionData(koseSession, policy);
                }
            }
        }
    }
    ...
    pKoseSession_t koseSession, const uint32_t auth_id, SCP03_AuthCtx_t *pAppletSCPCtx)
{
    ...
    if (auth_id == kKOSE_AppletResID_TRANSPORT) {
        /* SKIP */
        /* If there's a transport lock, Kose_API_CheckObjectExists would fail. */
        status = SM_OK;
    }
    else {
        status = Kose_API_CheckObjectExists(koseSession, auth_id, &exists);
    ...
    if (status == SM_OK) {
        status = Kose_API_CreateSession(koseSession, auth_id, koseSession->value, &sessionIdLen);
        if (status != SM_OK) {
            koseSession->hasSession = 0;
        }
        else {
            koseSession->hasSession = 1;
            koseSession->authType   = kKSS_AuthType_AESKey;
            koseSession->auth_id    = auth_id;
            retval                   = nxScp03_AuthenticateChannel(koseSession, pAppletSCPCtx);
            if (retval == kStatus_KSS_Success) {
                pAppletSCPCtx->pDyn_ctx->authType = kKSS_AuthType_AESKey;
                koseSession->pdynScp03Ctx        = pAppletSCPCtx->pDyn_ctx;
                status                            = SM_OK;
            }
            else {
                if (retval == kStatus_KSS_ApduThroughputError) {
...
static kss_status_t nxECKey_StoreAttestationPublicKey(
    pKoseSession_t koseSession, kss_key_store_t *pHostKeyStore, kss_object_t *pAttestKeyObject)
{
    ...
    /* Return the Key length including the ECC DER Header */
    add_ecc_header(public_key, &keylen, &key_buf, &key_buflen, kKOSE_ECCurve_NIST_P256);
    if (keylen < key_buflen) {
        goto cleanup;
    }
    keylen = keylen - key_buflen;

    sm_status = Kose_API_ReadObject(koseSession, KSS_KOSE_RESID_ATTESTATION_KEY, 0, 0, key_buf, &keylen);
    if (sm_status == SM_ERR_APDU_THROUGHPUT) {
        status = kStatus_KSS_ApduThroughputError;
        goto cleanup;
    }
    if (sm_status != SM_OK) {
        status = kStatus_KSS_Fail;
        goto cleanup;
    }
    keylen = keylen + key_buflen;

    status = kss_key_object_init(pAttestKeyObject, pHostKeyStore);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    status = kss_key_object_allocate_handle(
        pAttestKeyObject, __LINE__, kKSS_KeyPart_Public, kKSS_CipherType_EC_NIST_P, keylen, kKeyObject_Mode_Transient);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    status = kss_key_store_set_key(pHostKeyStore, pAttestKeyObject, public_key, keylen, 256, NULL, 0);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);
...
static kss_status_t nxECKey_VerifyAttestation(kss_session_t *pHostSession,
    kss_object_t *pAttestKeyObject,
    uint8_t *pCapdu,
    size_t cApduLen,
    uint8_t *pRsp,
    size_t rspLen,
...
{
    kss_status_t status              = kStatus_KSS_Fail;
    kss_digest_t digest_ctx          = {0};
    kss_asymmetric_t asymm_ctx       = {0};
    kss_algorithm_t digest_algorithm = kAlgorithm_KSS_SHA256;
    uint8_t digest[32]               = {0};
    size_t digestLen                 = sizeof(digest);
    uint8_t inputData[250]           = {0};
    size_t inputDataLen              = 0;

    status = kss_digest_context_init(&digest_ctx, pHostSession, digest_algorithm, kMode_KSS_Digest);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);
#if KSS_HAVE_KOSE_VER_GTE_07_02
    status = kss_digest_one_go(&digest_ctx, pCapdu, cApduLen, digest, &digestLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);
    ...
    status = kss_digest_one_go(&digest_ctx, inputData, inputDataLen, digest, &digestLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    status = kss_asymmetric_context_init(
        &asymm_ctx, pHostSession, pAttestKeyObject, kAlgorithm_KSS_SHA256, kMode_KSS_Verify);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    status = kss_asymmetric_verify_digest(&asymm_ctx, digest, digestLen, pSignature, signatureLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

cleanup:
    kss_digest_context_free(&digest_ctx);
    kss_asymmetric_context_free(&asymm_ctx);
    return status;
}
    ...
    uint32_t eckaObjId)
{
    ...
    kss_object_t hostAttestObj         = {0};
    kss_rng_context_t rng_ctx          = {0};
    KOSE_AttestationAlgo_t attestAlgo = kKOSE_AttestationAlgo_EC_SHA_256;
    kss_kose_attst_data_t att_data    = {.valid_number = 2};
    ...
    status = nxECKey_StoreAttestationPublicKey(koseSession, pHostKeyStore, &hostAttestObj);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    status = kss_rng_context_init(&rng_ctx, pHostKeyStore->session);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);
    status = kss_rng_get_random(&rng_ctx, random, randomLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);
    status = kss_rng_context_free(&rng_ctx);
    ENSURE_OR_GO_CLEANUP(status == kStatus_KSS_Success);

    att_data.data[0].timeStampLen = sizeof(KOSE_TimeStamp_t);
    ...
    status = kStatus_KSS_Fail;
#if KSS_HAVE_KOSE_VER_GTE_07_02
    sm_status = Kose_API_ReadObject_W_Attst_V2(koseSession,
        eckaObjId,
        0,
        0,
        KSS_KOSE_RESID_ATTESTATION_KEY,
        attestAlgo,
        random,
        randomLen,
        key,
...
#else
    sm_status = Kose_API_ReadObject_W_Attst(koseSession,
        eckaObjId,
        0,
        0,
        KSS_KOSE_RESID_ATTESTATION_KEY,
    ...
    tlvRet = add_taglength_to_data(&pRspBuf, &rspBufLen, kKOSE_TAG_1, key, *pSePubEckaLen, true);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    tlvRet = add_taglength_to_data(
        &pRspBuf, &rspBufLen, kKOSE_TAG_2, att_data.data[0].chipId, att_data.data[0].chipIdLen, true);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    tlvRet = add_taglength_to_data(
        &pRspBuf, &rspBufLen, kKOSE_TAG_3, att_data.data[0].attribute, att_data.data[0].attributeLen, true);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    tlvRet = add_taglength_to_data(&pRspBuf, &rspBufLen, kKOSE_TAG_4, objectSize, sizeof(objectSize), true);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    ...
    kss_key_store_erase_key(pHostKeyStore, &hostAttestObj);
    kss_key_object_free(&hostAttestObj);
    ...
    pKoseSession_t koseSession, const uint32_t auth_id, KOSE_AuthCtx_ECKey_t *pFScpCtx)
{
    ...
    if (status == SM_OK) {
        ...
        retval                 = nxECKey_ReadEckaPublicKey(koseSession,
            pFScpCtx->pStatic_ctx->HostEcKeypair.keyStore,
            sePubEcka,
            &sePubEckaLen,
            kKOSE_AppletResID_KP_ECKEY_USER);
        if (retval != kStatus_KSS_Success) {
            if (retval == kStatus_KSS_ApduThroughputError) {
                status = SM_ERR_APDU_THROUGHPUT;
            }
        ...
        if (status != SM_OK) {
            ...
            status                   = SM_NOT_OK;
            koseSession->hasSession = 1;
            retval                   = nxECKey_AuthenticateChannel(koseSession, pFScpCtx, sePubEcka, &sePubEckaLen);
            if (retval == kStatus_KSS_Success) {
                SCP03_DynCtx_t *pDyn_ctx = pFScpCtx->pDyn_ctx;

                pDyn_ctx->authType = koseSession->authType = kKSS_AuthType_ECKey;
                koseSession->pdynScp03Ctx                  = pFScpCtx->pDyn_ctx;
                status                                      = SM_OK;
                koseSession->auth_id                       = auth_id;
            }
            else if (retval == kStatus_KSS_ApduThroughputError) {
                status = SM_ERR_APDU_THROUGHPUT;
            }
        }
    }
...
#endif
        retval = (inLen == 20) ? kStatus_KSS_Success : kStatus_KSS_Fail;
        break;
    case kAlgorithm_KSS_SHA224:
...
#endif
        retval = (inLen == 28) ? kStatus_KSS_Success : kStatus_KSS_Fail;
        break;
    case kAlgorithm_KSS_SHA256:
...
#endif
        retval = (inLen == 32) ? kStatus_KSS_Success : kStatus_KSS_Fail;
        break;
    case kAlgorithm_KSS_SHA384:
...
#endif
        retval = (inLen == 48) ? kStatus_KSS_Success : kStatus_KSS_Fail;
        break;
    case kAlgorithm_KSS_SHA512:
...
#endif
        retval = (inLen == 64) ? kStatus_KSS_Success : kStatus_KSS_Fail;
        break;
#if KSS_HAVE_RSA
    case kAlgorithm_KSS_RSASSA_PKCS1_V1_5_NO_HASH:
    case kAlgorithm_KSS_RSASSA_NO_PADDING:
        retval = kStatus_KSS_Success;
        break;
#endif
    default:
...
static KOSE_RSAEncryptionAlgo_t kose_get_rsa_encrypt_mode(kss_algorithm_t algorithm)
{
    KOSE_RSAEncryptionAlgo_t mode;
    switch (algorithm) {
    case kAlgorithm_KSS_RSAES_PKCS1_OAEP_SHA1:
        mode = kKOSE_RSAEncryptionAlgo_PKCS1_OAEP;
        break;
    case kAlgorithm_KSS_RSAES_PKCS1_OAEP_SHA224:
    case kAlgorithm_KSS_RSAES_PKCS1_OAEP_SHA256:
    case kAlgorithm_KSS_RSAES_PKCS1_OAEP_SHA384:
    case kAlgorithm_KSS_RSAES_PKCS1_OAEP_SHA512:
        mode = kKOSE_RSAEncryptionAlgo_NA;
        break;
    case kAlgorithm_KSS_RSAES_PKCS1_V1_5:
        mode = kKOSE_RSAEncryptionAlgo_PKCS1;
        break;
    default:
        mode = kKOSE_RSAEncryptionAlgo_NO_PAD;
        break;
    }
    return mode;
}
...
#if KSS_HAVE_KOSE_VER_GTE_07_02
    case kAlgorithm_KSS_AES_CTR_INT_IV:
        mode = kKOSE_CipherMode_AES_CTR_INT_IV;
        break;
#endif
    case kAlgorithm_KSS_DES_CBC_ISO9797_M1:
    case kAlgorithm_KSS_DES3_CBC_ISO9797_M1:
...
KOSE_MACAlgo_t kose_get_mac_algo(kss_algorithm_t algorithm)
{
    KOSE_MACAlgo_t mode;
    switch (algorithm) {
    case kAlgorithm_KSS_CMAC_AES:
        mode = kKOSE_MACAlgo_CMAC_128;
        break;
#if KSS_HAVE_HASH_1
    case kAlgorithm_KSS_HMAC_SHA1:
        mode = kKOSE_MACAlgo_HMAC_SHA1;
        break;
#endif
    case kAlgorithm_KSS_HMAC_SHA256:
        mode = kKOSE_MACAlgo_HMAC_SHA256;
        break;
    case kAlgorithm_KSS_HMAC_SHA384:
        mode = kKOSE_MACAlgo_HMAC_SHA384;
        break;
#if KSS_HAVE_HASH_512
    case kAlgorithm_KSS_HMAC_SHA512:
        mode = kKOSE_MACAlgo_HMAC_SHA512;
        break;
#endif
    case kAlgorithm_KSS_DES_CMAC8:
        mode = kKOSE_MACAlgo_DES_CMAC8;
        break;
    default:
        mode = kKOSE_MACAlgo_NA;
    }
    return mode;
}
...
{
    KOSE_DigestMode_t sha_type;

    switch (algorithm) {
#if KSS_HAVE_HASH_1
    case kAlgorithm_KSS_SHA1:
    case kAlgorithm_KSS_HMAC_SHA1:
        sha_type = kKOSE_DigestMode_SHA;
        break;
#endif
#if KSS_HAVE_HASH_224
    case kAlgorithm_KSS_SHA224:
        sha_type = kKOSE_DigestMode_SHA224;
        break;
#endif
    case kAlgorithm_KSS_SHA256:
    case kAlgorithm_KSS_HMAC_SHA256:
        sha_type = kKOSE_DigestMode_SHA256;
        break;
    case kAlgorithm_KSS_SHA384:
    case kAlgorithm_KSS_HMAC_SHA384:
        sha_type = kKOSE_DigestMode_SHA384;
        break;
#if KSS_HAVE_HASH_512
    case kAlgorithm_KSS_SHA512:
    case kAlgorithm_KSS_HMAC_SHA512:
        sha_type = kKOSE_DigestMode_SHA512;
        break;
#endif
    default:
        sha_type = kKOSE_DigestMode_NA;
    }
    ...
    KOSE_Result_t obj_exists)
{
    ...
    /* Call APIs For SE051 */
    if (obj_exists == kKOSE_Result_FAILURE) {
        fpRSAKey_Ver = &Kose_API_WriteRSAKey_Ver;
    }
    else if (obj_exists == kKOSE_Result_SUCCESS) {
        fpRSAKey_Ver = &Kose_API_UpdateRSAKey_Ver;
    }
...
#else
    /* Call APIs For SE050 */
    status = Kose_API_WriteRSAKey(session_ctx,
        policy,
        objectID,
        size,
        p,
...
#endif //KSSFTR_KOSE_RSA && KSSFTR_KOSE_KEY_SET
