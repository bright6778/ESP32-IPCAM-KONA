...
kss_status_t kss_util_asn1_rsa_parse_private(const uint8_t *key,
    size_t keylen,
    kss_cipher_type_t cipher_type,
    uint8_t **modulus,
    size_t *modlen,
    uint8_t **pubExp,
    size_t *pubExplen,
    ...
    size_t *coefficientlen)
{
    ...
    if (modlen != NULL) {
        ...
        *modulus = KSS_MALLOC(*modlen);
        if ((*modulus != NULL) && ((*modlen) > 0)) {
            ...
            LOGE(TAG, "Either malloc failed or improper length");
            goto exit;
        }
    }
    else {
    ...
    if (exponent1len != NULL) {
        ...
        *exponent1 = KSS_MALLOC(*exponent1len);
    ...
    if (exponent2len != NULL) {
        ...
        *exponent2 = KSS_MALLOC(*exponent2len);
        if (*exponent2 == NULL) {
            LOGE(TAG, "malloc failed");
            goto exit;
        }
        if (*exponent2len > 0) {
            ...
            LOGE(TAG, "exponent2len improper");
            goto exit;
        }
    }
    else {
    ...
    if (coefficientlen != NULL) {
        ...
        *coefficient = KSS_MALLOC(*coefficientlen);
    ...
    status = kStatus_KSS_Success;
exit:
    return status;
}

kss_status_t kss_util_asn1_rsa_parse_private_allow_invalid_key(const uint8_t *key,
    size_t keylen,
    kss_cipher_type_t cipher_type,
    uint8_t **modulus,
    size_t *modlen,
...
kss_status_t kss_util_asn1_rsa_parse_public_nomalloc(
...
kss_status_t kss_util_asn1_rsa_parse_public_nomalloc_complete_modulus(
    const uint8_t *key, size_t keylen, uint8_t *modulus, size_t *modlen, uint8_t *pubExp, size_t *pubExplen)
{
    uint8_t *pBuf   = (uint8_t *)key;
    size_t taglen   = 0;
...
kss_status_t kss_util_asn1_rsa_parse_public(
    const uint8_t *key, size_t keylen, uint8_t **modulus, size_t *modlen, uint8_t **pubExp, size_t *pubExplen)
{
    uint8_t *pBuf   = (uint8_t *)key;
    size_t taglen   = 0;
...
kss_status_t kss_util_asn1_rsa_get_public(
    uint8_t *key, size_t *keylen, uint8_t *modulus, size_t modlen, uint8_t *pubExp, size_t pubExplen)
{
    ...
    if (*keylen < pbkeylen) {
        LOGE(TAG, "Buffer not sufficient");
        goto exit;
    }
...
kss_status_t kss_util_asn1_get_oid_from_header(uint8_t *input, size_t inLen, uint32_t *output, uint8_t *outLen)
{
    size_t i            = 0;
    size_t taglen       = 0;
    int objectIdCnt     = 0;
...
kss_status_t kss_util_asn1_get_oid_from_kssObj(kss_object_t *pkeyObject, uint32_t *output, uint8_t *outLen)
{
    kss_status_t status  = kStatus_KSS_Fail;
    uint8_t pbKey[256]   = {0};
    ...
    status = kss_util_asn1_get_oid_from_header(pbKey, pbKeyBytetLen, output, outLen);
    if (status != kStatus_KSS_Success) {
        goto exit;
    }
...
kss_status_t kss_util_pkcs8_asn1_get_ec_public_key_index(
...
kss_status_t kss_util_pkcs8_asn1_get_ec_pair_key_index(const uint8_t *input,
    size_t inLen,
...
kss_status_t kss_util_rfc8410_asn1_get_ec_pair_key_index(const uint8_t *input,
    size_t inLen,
    uint16_t *pubkeyIndex,
    size_t *publicKeyLen,
    uint16_t *prvkeyIndex,
...
kss_status_t kss_util_openssl_write_pkcs12(const char *pkcs12_cert,
    const char *password,
    const char *ref_key,
    long ref_key_length,
    const char *cert,
