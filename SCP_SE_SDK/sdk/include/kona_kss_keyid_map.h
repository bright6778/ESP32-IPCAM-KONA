...
#ifndef KSS_INC_KEYID_MAP_H_
#define KSS_INC_KEYID_MAP_H_
...
typedef struct
{
    ...
    /* Of type kss_key_part_t
     *
     * B0,B1,B2,B3 -> Key part  and  B4,B5,B6,B7 -> (No of slots taken - 1) */
    uint8_t keyPart;
    uint8_t accessPermission;
    uint8_t cipherType; /* Of type kss_cipher_type_t */
    /** Internal index */
    uint8_t keyIntIndex;
 ...
 * @param[in] kss_key The key object.
 ...
 */
kss_status_t ks_common_check_available_int_index(keyStoreTable_t *keystore_shadow,
    uint8_t object_type,
    uint8_t cipher_type,
    uint16_t *next_free_index,
    uint16_t keyLen);

kss_status_t ks_common_extId_to_int_index(keyStoreTable_t *keystore_shadow, uint32_t extId, uint16_t *intIndex);
/**
...
*/
kss_status_t ks_common_get_keyType_from_keyid(
...
void ks_sw_getKeyFileName(
    char *const file_name, const size_t size, const kss_object_t *kss_key, const char *root_folder);
kss_status_t ks_sw_fat_load(const char *szRootPath, keyStoreTable_t *pKeystore_shadow);

#endif /* KSS_INC_KEYID_MAP_H_ */
