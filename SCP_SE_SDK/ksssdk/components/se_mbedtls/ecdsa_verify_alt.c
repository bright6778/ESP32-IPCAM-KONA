...
{
    ecdsa_verify_ksskeystore = ksskeystore;
}
    ...
    const mbedtls_mpi *s)
{
    ...
    if (ecdsa_verify_ksskeystore != NULL) {
        kss_cipher_type_t cipherType = kKSS_CipherType_NONE;
        kss_object_t kssKeyObject    = {
            0,
        };
        kss_status_t status;
        size_t keyBitLen       = 0;
        ...
        */
        ...
        case MBEDTLS_ECP_DP_BP256R1:
            ...
            cipherType   = kKSS_CipherType_EC_BRAINPOOL;
            keyBitLen    = 256;
            break;
        ...
        case MBEDTLS_ECP_DP_SECP192K1:
            ...
            cipherType   = kKSS_CipherType_EC_NIST_K;
            keyBitLen    = 192;
            break;
        ...
        status = kss_key_object_init(&kssKeyObject, ecdsa_verify_ksskeystore);
        ...
        status =
            kss_key_store_set_key(ecdsa_verify_ksskeystore, &kssKeyObject, publickey, publickeylen, keyBitLen, NULL, 0);
        if (status != kStatus_KSS_Success) {
            return 1;
        }
        ...
        status = kss_asymmetric_context_init(
            &asymVerifyCtx, ecdsa_verify_ksskeystore->session, &kssKeyObject, algorithm, kMode_KSS_Verify);
        if (status != kStatus_KSS_Success) {
            return 1;
        }

        LOGD(TAG, "Verify using kss_asymmetric_verify_digest \n");
        status = kss_asymmetric_verify_digest(&asymVerifyCtx, (uint8_t *)buf, blen, (uint8_t *)signature, sigLen);
        if (status != kStatus_KSS_Success) {
            return 1;
        }
/*
