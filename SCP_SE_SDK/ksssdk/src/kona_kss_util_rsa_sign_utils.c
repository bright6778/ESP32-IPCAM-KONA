...
#if KSS_HAVE_APPLET_KOSE_IOT && KSSFTR_RSA && !KSS_HAVE_HOSTCRYPTO_NONE
...
static const char *TAG = "kona_kss_util_rsa_sign_utils.c";
...
*/
    ...
    if (outlength < (hashlen + oid_size + 6 /* DigestInfo TLV overhead */)) {
        LOGE(TAG, "Intended encoded message length too short");
        return 1;
    }

    if (*outLen < outlength) {
        LOGE(TAG, "Out buffer memory is less ");
        return 1;
    }
    *outLen = outlength;
